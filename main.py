...
if __name__ == "__main__":
    ...
    if not file_list:
        ...
        for filename in file_list:
            X_train, Y_train, X_test, Y_test, X_full, Y_full = load_and_split_data(
                filename
            )
            ...
            nn_batch_tanh = SimpleNeuralNet(
                hidden_size=hidden_size_to_use, activation="tanh"
            )
            nn_batch_tanh.train(
                X_train, Y_train, epochs=epochs_to_use, learning_rate=0.05
            )
            ...
            current_results["Batch_Tanh"] = {
                "Train MSE": train_mse_bt,
                "Test MSE": test_mse_bt,
                "model": nn_batch_tanh,
            }

            nn_batch_relu = SimpleNeuralNet(
                hidden_size=hidden_size_to_use, activation="relu"
            )
            nn_batch_relu.train(
                X_train, Y_train, epochs=epochs_to_use, learning_rate=0.01
            )
            ...
            current_results["Batch_ReLU"] = {
                "Train MSE": train_mse_br,
                "Test MSE": test_mse_br,
                "model": nn_batch_relu,
            }

            nn_online_tanh = SimpleNeuralNet(
                hidden_size=hidden_size_to_use, activation="tanh"
            )
            nn_online_tanh.train(
                X_train,
                Y_train,
            ...
            current_results["Online_Tanh"] = {
                "Train MSE": train_mse_ot,
                "Test MSE": test_mse_ot,
                "model": nn_online_tanh,
            }

            nn_online_relu = SimpleNeuralNet(
                hidden_size=hidden_size_to_use, activation="relu"
            )
            nn_online_relu.train(
                X_train, Y_train, epochs=epochs_to_use, learning_rate=0.01
            )
            ...
            current_results["Online_ReLU"] = {
                "Train MSE": train_mse_or,
                "Test MSE": test_mse_or,
                "model": nn_online_relu,
            }
            ...
            print(
                f"  Batch+Tanh:  Train MSE: {train_mse_bt:.4f}, Test MSE: {test_mse_bt:.4f}"
            )
            print(
                f"  Batch+ReLU:  Train MSE: {train_mse_br:.4f}, Test MSE: {test_mse_br:.4f}"
            )
            print(
                f"  Online+Tanh: Train MSE: {train_mse_ot:.4f}, Test MSE: {test_mse_ot:.4f}"
            )
            print(
                f"  Online+ReLU: Train MSE: {train_mse_or:.4f}, Test MSE: {test_mse_or:.4f}"
            )
            print("-" * 30)
        ...
        if file_list and results:
            ...
            for filepath in file_list:
                ...
                for ax, (model_name, m_results) in zip(axes_flat, file_models.items()):
                    ...
                    ax.plot([], [], " ", label=f"Test MSE: {test_mse:.4f}")

                    ax.legend()
                    ax.grid(True, linestyle="--", alpha=0.7)

                fig.suptitle(
                    f"Dataset: {filepath.name}", fontsize=16, fontweight="bold"
                )
